 ...
 * @author Tuanbm2
 */
class sfWidgetFormOneMany extends sfWidgetForm
{
...
//        $relateType =  "ONE2MANY";
        ...
        <a href="#vtModal_{input.id}" role="button" class="btn" data-toggle="modal" id="{attr.id}">{input.button_text}</a>
    ...
    public function render($name, $value = null, $attributes = array(), $errors = array())
    {
        ...
        foreach ($dataQueryList as $record) {
            ...
            foreach ($visibleColumns as $column) {
                ...
                $tr = $tr . '<td><span id="xxxx" title="' . VtHelper::encodeOutput($record[$column], true) . '">' . VtHelper::getLimitStringWithoutEncode($record[$column], 100) . '</span></td>';
            }
            if ($orderColumnName != "") {
