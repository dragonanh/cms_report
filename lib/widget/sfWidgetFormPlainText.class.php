...
class sfWidgetFormPlainText extends sfWidgetForm {
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    if($encode==true){
      $output =  VtHelper::encodeOutput($output,true);
    }else{
      $output =  VtHelper::strip_html_default_tags($output);
    }

    return sprintf('<span %s>%s</span>', $this->attributesToHtml($attributes), $output);
  }
