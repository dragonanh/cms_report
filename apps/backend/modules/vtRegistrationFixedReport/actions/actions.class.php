...
require_once dirname(__FILE__) . '/../lib/vtRegistrationFixedReportGeneratorConfiguration.class.php';
require_once dirname(__FILE__) . '/../lib/vtRegistrationFixedReportGeneratorHelper.class.php';

/**
 * vtRegistrationFixedReport actions.
 *
 * @package    cms_ctt
 * @subpackage vtRegistrationFixedReport
 * @author     viettel
 * @version    SVN: $Id: actions.class.php_bak.bak 23810 2009-11-12 11:07:44Z Kris.Wallsmith $
 */
class vtRegistrationFixedReportActions extends autoVtRegistrationFixedReportActions
{
  public function executeFilter(sfWebRequest $request)
  {
    ...
    if ($request->hasParameter('_reset')) {
      ...
      $this->redirect('@vt_ctt_transaction_vtRegistrationFixedReport');
    }
  ...
  public function exportExcel()
  {
    ...
    $sheet->setCellValue('A1', 'Báo cáo giao dịch đăng ký cố định');
...
//        $sheet->getStyle('A3')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
    ...
    $sheet->setCellValue('F3', $i18n->__('Tổng tiền'));
    $sheet->setCellValue('G3', $i18n->__('Chính sách đóng cước trước'));
    $sheet->setCellValue('H3', $i18n->__('Thời gian đăng ký'));
    $sheet->setCellValue('I3', $i18n->__('Mã thanh toán (Mã sinh ra từ MyVT)'));
    $sheet->setCellValue('J3', $i18n->__('Mã giao dịch (Mã sinh ra từ Cổng thanh toán)'));
    $sheet->setCellValue('K3', $i18n->__('Trạng thái trừ tiền'));
    ...
    foreach ($results as $key => $result) {
      ...
      $sheet->setCellValue("F$startRow", $result->getTotalFee());
      $sheet->setCellValue("G$startRow", $result->getPolicy());
      $sheet->setCellValue("H$startRow", $result->getCreatedAt());
      $sheet->setCellValue("I$startRow", $result->getTranId());
      $sheet->setCellValue("J$startRow", $result->getCttId());
      $sheet->setCellValue("K$startRow", $result->getStatusName());
    }
    ...
    $fileName = 'registration_fixed_report_' . date('YmdHis') . '.xlsx';
